...
def write_graph_info_to_file(G, graph_name=GRAPH_NAME):
    with open("{}_info".format(graph_name), "w") as report_file:
        try:
            ...
            report_file.write("Unexpected error: {}".format(sys.exc_info()[0]))
...
def write_pagerank_graph(G, n):
    ...
    nx.write_edgelist(best_subgraph, "{}_best_{}_pagerank.csv".format(GRAPH_NAME, n), delimiter=';')
...
def write_strong_components_graphs(G, n):
    ...
    for i, biggest_component in enumerate(biggest_components):
        ...
        nx.write_edgelist(subgraph, "{}_biggest_strongly_connected_component_{}_{}.csv".format(GRAPH_NAME, i + 1, n), delimiter=';')
...
def write_weak_components_graphs(G, n):
    ...
    for i, biggest_component in enumerate(biggest_components):
        subgraph = G.subgraph(biggest_component)
        nx.write_edgelist(subgraph, "{}_biggest_weakly_connected_component_{}_{}.csv".format(GRAPH_NAME, i + 1, n), delimiter=';')
